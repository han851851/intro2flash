...
To save your workspace so you don't have to select waves and zoom into a specific range each time you open your waveforms, press `ctrl+s` to save your workspace. The first time you do this, make sure to save it with the same name as your module name. Now, every time you use a simulation target, the view with all waves you selected will pop up automatically!

### File naming conventions
Make sure all `<module-name>`s within a single file, testbench, and waveform combo are the same.
- Source    (file and module name): `<module-name>.sv`
- Testbench (file and module name: `<module-name>_tb.sv`
- Dumpfile: `waves/<module-name>.vcd`
- GTKWave workspace: `<module-name>.gtkw>`
...
This template is configured to target the iCE40HX8K FPGAs (the same as the ECE270 development boards) which you will all hopefully use further in the semester for testing. To do this, modify the `top` file inside of the `source` folder without changing the top-level inputs and outputs or module name. This module will be the wrapper that allows your designs to be flashed or crammed onto the FPGA.
To do this, use `make cram` for volitile configuration. If you need the design to remain after you remove power from the FPGA, use `make flash`. We suggest you use the former
